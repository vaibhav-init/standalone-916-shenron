...
# sys.path.append('/radar-imaging-dataset/carla-radarimaging/team_code/sim_radar_utils/')
  ...
  def __init__(self,
    ...
    for sub_root in tqdm(root, file=sys.stdout, disable=rank != 0):
      ...
      for route in routes:
        ...
        # TEMPORARILY lowered to 0.0 to accept all routes for testing with partial data
        if results_route['scores']['score_composed'] < 0.0:
          continue
    ...
    self.radars_back = np.array(self.radars_back).astype(np.string_)
    self.radars_left = np.array(self.radars_left).astype(np.string_)
    self.radars_right = np.array(self.radars_right).astype(np.string_)
